...
FROM `northwind`.`order_details` INNER JOIN `northwind`.`orders` ON
`northwind`.`order_details`.`id` = `northwind`.`orders`.`id` 
INNER JOIN `northwind`.`orders_status` ON `northwind`.`order_details`.`id` = `northwind`.`orders_status`.`id`
INNER JOIN `northwind`.`order_details_status` ON `northwind`.`order_details`.`id` = `northwind`.`order_details_status`.`id`; 
...
SELECT northwind_dw.dim_customers.last_name, northwind_dw.fact_orders.quantity, northwind_dw.fact_orders.unit_price FROM northwind_dw.dim_customers 
